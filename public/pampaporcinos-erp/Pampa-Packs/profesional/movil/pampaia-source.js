(function () {
  ...
  function getStoredLicense() {
    ...
    for (const key of keys) {
      try {
        ...
        if (!expiration || new Date(expiration).getTime() > Date.now()) return license;
      } catch {}
    }
    return null;
  }
  ...
  function getActiveTrial() {
    ...
    return license && (license.type === 'trial' || license.payload?.trial) ? license : null;
  }

  function getAiPlan() {
    if (getActiveTrial()) return 'trial';
    const license = getStoredLicense() || {};
    const direct = String(license.plan || license.version || license.payload?.plan || '').toLowerCase();
    if (['basica', 'profesional', 'premium'].includes(direct)) return direct;
    const code = String(license.code || license.key || '').match(/^NEXO-([BPM])/i)?.[1]?.toUpperCase();
    return ({ B: 'basica', P: 'profesional', M: 'premium' })[code] || 'basica';
  }
  ...
  function mountPlanCapabilities(panel) {
    ...
    const currentLevel = levels[plan] || 1;
    const status = (required, label) => plan === 'trial' ? 'Vista previa del trial' : currentLevel >= required ? 'Incluido en tu plan' : 'Requiere ' + label;
    const box = document.createElement('section');
    box.id = 'pampaIAPlanCapabilities';
    box.style.cssText = 'position:relative;z-index:1;margin-top:14px;padding:14px;border:1px solid rgba(159,192,173,.35);border-radius:10px;background:rgba(0,0,0,.14);';
    box.innerHTML = '<div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap"><div style="font-size:10px;color:#9fc0ad;letter-spacing:.9px;font-family:var(--font-mono);text-transform:uppercase">Capacidades de IA por versión</div><span style="font-size:11px;color:#fbbf24">Plan actual: ' + ({ basica: 'Básica', profesional: 'Profesional', premium: 'Premium', trial: 'Trial completo' })[plan] + '</span></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px;margin-top:10px"><article style="padding:12px;border-radius:8px;background:rgba(255,255,255,.04)"><strong style="color:#fff">SMALL · Básica</strong><p style="margin:7px 0;color:#d6e5db;font-size:12px;line-height:1.45">Consultas sobre datos locales, resúmenes diarios y alertas de desvíos simples de precio, stock o rinde.</p><small style="color:#9fc0ad">' + status(1, 'Básica') + '</small></article><article style="padding:12px;border-radius:8px;background:rgba(255,255,255,.04)"><strong style="color:#fff">MEDIUM · Profesional</strong><p style="margin:7px 0;color:#d6e5db;font-size:12px;line-height:1.45">Proyección de quiebres de stock, costos por lote y mantenimiento según consumos, horas y campañas cargadas.</p><small style="color:#9fc0ad">' + status(2, 'Profesional') + '</small></article><article style="padding:12px;border-radius:8px;background:rgba(255,255,255,.04)"><strong style="color:#fff">LARGE · Premium</strong><p style="margin:7px 0;color:#d6e5db;font-size:12px;line-height:1.45">Zonificación, prescripción variable y comparativas entre establecimientos con mapas e historial suficiente.</p><small style="color:#9fc0ad">' + status(3, 'Premium') + '</small></article></div><p style="margin:10px 0 0;color:#9fc0ad;font-size:11px">Las predicciones requieren datos históricos compatibles. Los avisos por WhatsApp se envían únicamente cuando la cuenta oficial está configurada.</p>';
    panel.appendChild(box);
  }

  function mountTrialDemos(panel) {
    if (!getActiveTrial() || panel.querySelector('#pampaIATrialDemos')) return;
    const state = getState();
    const lot = (state.lotes || [])[0]?.codigo || (state.lotes || [])[0]?.nombre || 'lote demostrativo';
    const trial = document.createElement('section');
    trial.id = 'pampaIATrialDemos';
    trial.style.cssText = 'position:relative;z-index:1;margin-top:14px;padding:14px;border:1px solid rgba(245,158,11,.35);border-radius:10px;background:rgba(245,158,11,.08);';
    trial.innerHTML = '<div style="font-size:10px;color:#fbbf24;letter-spacing:.9px;font-family:var(--font-mono);text-transform:uppercase">Demo de IA para trial</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin-top:10px"><article style="padding:12px;border-radius:8px;background:rgba(0,0,0,.16)"><strong style="color:#fff">🎙️ Voz a datos</strong><p style="margin:7px 0;color:#d6e5db;font-size:12px;line-height:1.45">Ejemplo: “Anotá 40 litros de gasoil en el lote”.</p><div style="display:flex;gap:7px;flex-wrap:wrap"><button type="button" class="btn" data-trial-voice-play>🔊 Escuchar</button><button type="button" class="btn" data-trial-voice-process>Procesar</button></div><div data-trial-voice-result style="margin-top:9px;color:#b8d1c2;font-size:12px"></div></article><article style="padding:12px;border-radius:8px;background:rgba(0,0,0,.16)"><strong style="color:#fff">📷 Escaneo de remito</strong><p style="margin:7px 0;color:#d6e5db;font-size:12px;line-height:1.45">Ejemplo de extracción de proveedor, comprobante e importe.</p><button type="button" class="btn" data-trial-scan>Ver extracción</button><div data-trial-scan-result style="margin-top:9px;color:#b8d1c2;font-size:12px"></div></article></div><div style="margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,255,255,.1)"><strong style="color:#fff;font-size:12px">IA por versión</strong><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:9px;margin-top:8px"><button type="button" class="btn" data-trial-ai="small">SMALL · Asistencial</button><button type="button" class="btn" data-trial-ai="medium">MEDIUM · Predictiva</button><button type="button" class="btn" data-trial-ai="large">LARGE · Prescriptiva</button></div><div data-trial-ai-result style="margin-top:9px;color:#b8d1c2;font-size:12px">Elegí una versión para ver su alcance.</div></div><p style="margin:11px 0 0;color:#9fc0ad;font-size:11px">Datos demostrativos. 🎤 Dictar transcribe tu voz; 🔊 Escuchar reproduce la locución de ejemplo. Cámara y OCR reales dependen de permisos, navegador y backend configurado.</p>';
    panel.appendChild(trial);
    const voiceResult = trial.querySelector('[data-trial-voice-result]');
    trial.querySelector('[data-trial-voice-play]').addEventListener('click', () => {
      const text = 'Anotá 40 litros de gasoil en el lote demostrativo.';
      if (!window.speechSynthesis || !window.SpeechSynthesisUtterance) {
        voiceResult.textContent = 'Este navegador no permite reproducir la locución de ejemplo.';
        return;
      }
      ...
      utterance.rate = 0.9;
      utterance.onstart = () => { voiceResult.textContent = 'Reproduciendo dictado demostrativo...'; };
      utterance.onend = () => { voiceResult.textContent = 'Dictado escuchado. Ahora podés procesarlo.'; };
      utterance.onerror = () => { voiceResult.textContent = 'No se pudo reproducir el dictado en este navegador.'; };
      window.speechSynthesis.speak(utterance);
    });
    trial.querySelector('[data-trial-voice-process]').addEventListener('click', () => {
      const record = { recordedAt: new Date().toISOString(), lot, input: '40 litros de gasoil', category: 'Combustible', demo: true };
      localStorage.setItem('pampaia-trial-voice-demo', JSON.stringify(record));
      voiceResult.innerHTML = '<strong style="color:#fff">Dictado interpretado</strong><br>Lote: ' + lot + ' · Insumo: gasoil · Cantidad: 40 L · Categoría: combustible.<br><small>Registro demostrativo guardado localmente.</small>';
    });
    trial.querySelector('[data-trial-scan]').addEventListener('click', () => {
      trial.querySelector('[data-trial-scan-result]').innerHTML = '<strong style="color:#fff">Remito extraído</strong><br>Proveedor: Insumos del Campo<br>Comprobante: REM-0001-00452<br>Producto: Urea granulada<br>Cantidad: 8.750 kg<br>Importe: USD 6.920<br><small>Resultado demostrativo: revisar antes de registrar una operación real.</small>';
    });
    const aiResult = trial.querySelector('[data-trial-ai-result]');
    trial.querySelectorAll('[data-trial-ai]').forEach((button) => button.addEventListener('click', () => {
      const mode = button.dataset.trialAi;
      if (mode === 'small') aiResult.innerHTML = '<strong style="color:#fff">SMALL · IA asistencial</strong><br>Consulta datos locales, resume novedades y alerta desvíos simples. Incluye dictado y lectura de comprobantes sujetos a configuración.';
      if (mode === 'medium') aiResult.innerHTML = '<strong style="color:#fff">MEDIUM · IA predictiva</strong><br>Anticipa quiebres de stock, proyecta costos y recomienda mantenimiento según los datos históricos cargados.';
      if (mode === 'large') aiResult.innerHTML = '<strong style="color:#fff">LARGE · IA prescriptiva</strong><br>Genera zonificación, recomendaciones de dosis variable y comparativas multi-establecimiento cuando existen mapas e historial suficiente.';
    }));
  }
