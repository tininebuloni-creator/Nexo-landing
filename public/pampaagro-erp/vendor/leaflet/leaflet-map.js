...
(function (global) {
  ...
  function bootstrap(intentos) {
    ...
    // 2. Si no hay un contenedor configurado explícitamente, NO adivinamos con cualquier div
    //    que contenga "map" en el id — eso chocaba con el mapa de Lotes de la app (#mapaLotes).
    //    Para usar este script en otra función de seguimiento, configurá antes:
    //    window.LeafletMapConfig = { containerId: 'tuIdDeContenedor' };
    // (Verificación de fallback amplio desactivada intencionalmente)
    ...
    if (el) {
      ...
      // Si la app tarda mucho en renderizar la pantalla, reintentar en 400ms (máx. ~8s en total)
      setTimeout(function () { bootstrap(intentos + 1); }, 400);
    } else {
      log('No se encontró contenedor ("' + containerId + '") tras varios intentos. Se deja de buscar hasta que se configure window.LeafletMapConfig.');
    }
  }
