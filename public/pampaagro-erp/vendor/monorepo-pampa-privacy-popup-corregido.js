(function () {
  ...
  function showNotice() {
    ...
    // Solo mostrar este aviso en el trial. Se detecta por la licencia real guardada
    // (localStorage.nexoAgroLicense, tipo 'trial') y, como respaldo, por el modo
    // legado ?modo=trial de la URL. En la versión paga (licencia comprada / Electron
    // sin licencia trial) este cartel NO debe aparecer.
    let esTrial = false;
    try {
      const licenseStr = localStorage.getItem('nexoAgroLicense');
      const license = licenseStr ? JSON.parse(licenseStr) : null;
      esTrial = !!(license && license.type === 'trial');
    } catch (e) { /* si no se puede leer, no asumimos trial */ }
    if (!esTrial) {
      esTrial = document.body.classList.contains('modo-trial-activo') || new URLSearchParams(window.location.search).get('modo') === 'trial';
    }
    if (!esTrial) return;
