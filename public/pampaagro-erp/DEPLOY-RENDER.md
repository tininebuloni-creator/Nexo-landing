...
2. **Configuración automática:**
   ...
   - Nombre del servicio en este repo: `pampaagro-erp`
   - Runtime: Node.js
   - Build: `npm install`
   - Start: `npm start`
...
- El servidor web sirve `index.html` en la raíz (con fallback a `nexo-agro-erp.html` si existiera)
- API en `/api/*` para backend

## Cambio de nombre del servicio

Si cambiaste `name` en `render.yaml`, Render puede crear un servicio nuevo en lugar de renombrar el actual.

- Verificá en el panel de Render cuál servicio está conectado al repo
- Si aparecen dos servicios, mantené uno solo activo para evitar deploys duplicados
