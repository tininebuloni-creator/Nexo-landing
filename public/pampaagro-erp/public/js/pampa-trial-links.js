(function (global) {
  ...
  function showTrialBanner() {
    ...
    banner.textContent = `Prueba gratuita activa: quedan ${trial.remaining} día(s).`;
  ...
  function getActiveTrial() {
    ...
    for (const key of keys) {
      try {
        ...
        if (!license || (license.type !== 'trial' && !license.payload?.trial)) continue;
  ...
  function showTrialPrivacyNotice() {
    if (document.getElementById('trialPrivacyNotice') || !getActiveTrial()) return;
