...
// _license.js
var PUBLIC_KEY_B64 = "MCowBQYDK2VwAyEAq7tUZMWSHD1Q36bRapLV6adFdCUzm9mwrV94npOEW3c=";
...
async function verifyToken(token, expectedProduct) {
  const parts = String(token || "").trim().split(".");
  if (parts.length !== 3 || parts[0] !== "PAMPAN1") throw new Error("Licencia inv\xE1lida");
  const payloadBytes = decodeBase64Url(parts[1]);
  const signature = decodeBase64Url(parts[2]);
  const payload = JSON.parse(new TextDecoder().decode(payloadBytes));
  if (payload.product !== expectedProduct) throw new Error("Licencia emitida para otra aplicaci\xF3n");
  if (!Number.isInteger(payload.maxUsers) || payload.maxUsers < 1) throw new Error("La licencia no tiene cupo v\xE1lido");
  if (payload.expiresAt && (/* @__PURE__ */ new Date(`${payload.expiresAt}T23:59:59`)).getTime() < Date.now()) throw new Error("Licencia vencida");
  ...
  if (!valid) throw new Error("Firma de licencia inv\xE1lida");
  return payload;
}
...
function getStore(env2, licenseId) {
  if (!env2.LICENSE_ACTIVATIONS) throw new Error("Falta configurar el KV LICENSE_ACTIVATIONS");
  return env2.LICENSE_ACTIVATIONS.get(`license:${licenseId}`, "json");
}
__name(getStore, "getStore");
function putStore(env2, licenseId, value) {
  if (!env2.LICENSE_ACTIVATIONS) throw new Error("Falta configurar el KV LICENSE_ACTIVATIONS");
  return env2.LICENSE_ACTIVATIONS.put(`license:${licenseId}`, JSON.stringify(value));
}
__name(putStore, "putStore");

// v1/client/activate.js
...
async function onRequestPost({ request, env: env2 }) {
  try {
    ...
    const current = await getStore(env2, payload.licenseId) || { token: body.token, product: payload.product, maxUsers: payload.maxUsers, devices: [] };
    ...
    if (!devices.some((item) => item.deviceId === deviceId)) devices.push({ deviceId, activatedAt: (/* @__PURE__ */ new Date()).toISOString() });
    await putStore(env2, payload.licenseId, { token: body.token, product: payload.product, maxUsers: payload.maxUsers, devices });
    return json({ ok: true, licenseId: payload.licenseId, maxUsers: payload.maxUsers, activeUsers: devices.length, token: body.token });
  } catch (error3) {
    return json({ ok: false, message: error3.message || "No se pudo activar el dispositivo" }, 400);
  }
}
__name(onRequestPost, "onRequestPost");

// v1/client/activations.js
...
async function onRequestPost2({ request, env: env2 }) {
  try {
    const body = await request.json();
    const payload = await verifyToken(body.token, body.product || "pampa-agro");
    const current = await getStore(env2, payload.licenseId);
    ...
    return json({ ok: false, message: error3.message || "No se pudo listar activaciones" }, 400);
  }
}
__name(onRequestPost2, "onRequestPost");

// v1/client/authorization.js
...
async function onRequestGet({ request, env: env2 }) {
  try {
    ...
    const current = await getStore(env2, licenseId);
    ...
    return json({ ok: false, message: error3.message || "No autorizado" }, 403);
  }
}
__name(onRequestGet, "onRequestGet");

// v1/client/release.js
...
async function onRequestPost3({ request, env: env2 }) {
  try {
    ...
    await putStore(env2, payload.licenseId, { ...current, devices });
    return json({ ok: true, activeUsers: devices.length, maxUsers: payload.maxUsers });
  } catch (error3) {
    return json({ ok: false, message: error3.message || "No se pudo liberar el dispositivo" }, 400);
  }
}
__name(onRequestPost3, "onRequestPost");

// api/auditoria-fiscal.js
var AUDIT_PREFIX = "fiscal-audit:pampa-biogas:";
var CH4_TONNES_PER_M3 = 717e-6;
var METHANE_GWP = 28;
var CARBON_PRICE_USD = 25;
var DISPOSAL_RATE_USD_PER_TON = 12.5;
...
function number(value, name) {
  ...
  if (!Number.isFinite(parsed) || parsed < 0) throw new Error(`${name} inv\xE1lido`);
  return parsed;
}
__name(number, "number");
function onRequestOptions5() {
  return json2(null, 204);
}
...
async function onRequestPost4({ request, env: env2 }) {
  try {
    ...
    await env2.LICENSE_ACTIVATIONS.put(`${AUDIT_PREFIX}${auditId}`, JSON.stringify(record));
    return json2({ ok: true, auditId, record });
  } catch (error3) {
    return json2({ ok: false, message: error3.message || "No se pudo registrar la auditor\xEDa fiscal" }, 400);
  }
}
...
async function onRequestGet2({ request, env: env2 }) {
  try {
    ...
    if (!auditId) return json2({ ok: false, message: "Falta auditId" }, 400);
    const record = await env2.LICENSE_ACTIVATIONS.get(`${AUDIT_PREFIX}${auditId}`, "json");
    return record ? json2({ ok: true, record }) : json2({ ok: false, message: "Auditor\xEDa no encontrada" }, 404);
  } catch (error3) {
    return json2({ ok: false, message: error3.message || "No se pudo leer la auditor\xEDa fiscal" }, 400);
  }
}
__name(onRequestGet2, "onRequestGet");

// ../.wrangler/tmp/pages-LMaWhs/functionsRoutes-0.31907059894814527.mjs
var routes = [
  {
  ...
  {
    routePath: "/api/auditoria-fiscal",
    mountPath: "/api",
    method: "GET",
    middlewares: [],
